...
<body>
    ...
    <ul class='nav'>
      <li class='nav-item'>
        ...
        <div class='link' id='users'>
          ...
          <form action="admin.php" method="post"><input type='submit' class="link-text" name='options' value='Users'/></form>
        </div>
        <!-- </a> -->
      </li>
      <li class='nav-item'>
      ...
      <li class='nav-item'>
        <div class='link' id='system'>
          ...
          <form action="admin.php" method="post"><input type='submit' class="link-text" name='options' value='System'/></form>
        </div>
      </li>
      <li class='ins'>
        <div class="link">
  ...
  <?php include "assets/php/db_conn.php";
    $conn = new mysqli(DB_SERVER, DB_USERNAME, DB_PASSWORD, DB_NAME);
    if ($conn->connect_error) {
      die("Connection failed: " . $conn->connect_error);
    }

    function showUsers($conn, $query_user) {
      $all_users = mysqli_query($conn, $query_user);

      echo "<div class='tb' id='userinfo'>";
        echo "<h3>Users</h3>";
    ...
    function showStudios($conn, $query_studio) {
      $all_studios = mysqli_query($conn, $query_studio);
    ...
    try {
      if (isset($_POST['options'])) {
        switch( $_POST['options'] ) {
          ...
          case 'Studios':
            $query = "SELECT studios.*, users.displayName FROM studios left join users on studios.owner = users.email;";
            showStudios($conn, $query);
          break;

          case 'System':
            echo "<div class='tb' id='sysinfo'>";
            echo "<h3>Server and Database</h3>";
            echo "<h5>";
            ...
            echo "</h5>";
            echo "<h5>";
            printf("SQL Protocol Version:    %s\n", $conn->protocol_version);
            echo "</h5>";
            echo "<h5>";
            printf("SQL Host Info:    %s\n", $conn->host_info);
            echo "</h5>";
            echo "</div>";
          break;
          default:
